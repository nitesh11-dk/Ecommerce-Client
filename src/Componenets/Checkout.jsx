  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="text-center text-white mt-10">
        ...
        <Link to="/" className="btn text-xl btn-warning mt-4">
          Continue Shopping ...
        </Link>
      </div>
    );
    ...
    <div className="container mx-auto w-screen justify-center items-center p-6 text-white flex flex-col gap-8">
          ...
          <table className="  bg-gray-900  min-w-full table-auto text-left overflow-hidden  ">
            ...
            <tbody className="bg-gray-800">
              {cart.items.map((item) => (
                <tr key={item._id} className="border-b border-gray-600">
                  ...
                  <td className="px-4 py-7 flex justify-center items-center border border-gray-400">
                    {item.quantity}
                  </td>
                  <td className="px-4 py-2 border border-gray-400">
                  ...
                  <td className="px-4 py-2 border border-gray-400">
                    <button
                      onClick={() =>
                        isLoggedIn && addToCart(item.productId._id)
                      }
                  ...
                  <td className="px-4 py-2 border border-gray-400">
                    <button
                      onClick={() =>
                        isLoggedIn && decreaseQuantity(item.productId._id)
                      }
            ...
            <tfoot>
              <tr className="overflow-hidden">
                <td
                  colSpan="2"
                  className="border border-gray-400 px-4 py-2 text-lg font-bold text-end"
                >
                  Total
                </td>
                <td className="px-4 flex justify-center items-center py-2 border border-gray-400 text-lg font-bold">
                  {totalQuantity}
                </td>
                <td
                  colSpan="1"
                  className=" border border-gray-400px-4 py-2 text-lg font-bold text-green-400 text-center"
                >
      ...
      <h3 className="text-center  ">
        <button
          ...
          className="px-4 py-2 rounded-2xl bg-yellow-400  m-4 text-black font-semibold hover:bg-yellow-600"
        >
          Proceed To Pay{" "}
        </button>
      </h3>
